...
export async function deleteUser(req, res) {
  try {
    ...
    // 2. Delete all friend requests associated with this user
    await FriendRequest.deleteMany({
      $or: [{ sender: userIdToDelete }, { recipient: userIdToDelete }],
    });

    // 3. Remove this user from everyone's friends list
    await User.updateMany(
      { friends: userIdToDelete },
      { $pull: { friends: userIdToDelete } }
    );

    res.status(200).json({ message: "User and all associations deleted successfully" });
  } catch (error) {
